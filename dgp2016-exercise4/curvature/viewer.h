...
public:
    ...
    void uniform_laplacian_enhance_feature(int enhancement_smoothing_iterations,
                                           float enhancement_coef);

    void laplace_beltrami_enhance_feature(int enhancement_smoothing_iterations,
                                          float enhancement_coef);
    ...
    void meshProcess() {
        ...
        auto v_unicurvature = mesh.vertex_property<Scalar>("v:unicurvature", 0);
        ...
        color_coding(v_unicurvature, &mesh, v_color_unicurvature);
    ...
    void initGUI() {
        ...
        new Label(window, "Smoothing", "sans-bold");
        popupBtn = new PopupButton(window, "Smooth");
        popup = popupBtn->popup();
        popup->setLayout(new GroupLayout());
        b = new Button(popup, "Uniform Laplacian");
        b->setCallback([this]() {
            this->uniform_smooth(10);
            this->meshProcess();
        });
        b = new Button(popup, "Laplace-Beltrami");
        b->setCallback([this]() {
        ...
        popupBtn = new PopupButton(window, "Enhancement");
        popup = popupBtn->popup();
        popup->setLayout(new GroupLayout());
        ...
        b->setCallback([this]() {
            uniform_laplacian_enhance_feature(this->iterationTextBox->value(),
                                              this->coefTextBox->value());
            meshProcess();
        });
        b = new Button(panel, "Laplace-Beltrami");
        b->setCallback([this]() {
            laplace_beltrami_enhance_feature(this->iterationTextBox->value(),
                                              this->coefTextBox->value());
            meshProcess();
        });
