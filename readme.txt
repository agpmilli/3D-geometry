Exercise 2 :
	...
	LaplacianSmoothing() :
		...
		5) We compute the new point's coordinates separatly for points with indexes 0 and num_points-1 to avoid invalid indexes access.
	...
	OsculatingCircle() :
		...
		5) We compute the new point's coordinates using the formula given in the exercise and we store it in the temporary matrix. (points_tmp)
		5) We compute the new point's coordinates separatly for points with indexes 0 and num_points-1 to avoid invalid indexes access.
		6) We compute the new length of the curve by iterating through the temporary matrix. (add it to L1)
		7) At the end we replace the old points by the new ones and uniformly scale the curve back to its original length using L and L1. (we multiply the new points by L/L1)
